...
def proc_etapa0(df, type_proc='normal', stop_name=''):
    """
    Función que define cómo se define la capa 1. 
    ...
    if type_proc == 'pos_neu_neg':
         df['etiqueta_etapa0'] = df['etiqueta_final'].replace({"POSITIVO" : 'R', 
                                                          ...
                                                          "NECESIDADES OPERATIVAS": 'NR',
                                                          "NO RELEVANTE" : 'NR'})
    if type_proc == 'neg_necop':
         df['etiqueta_etapa0'] = df['etiqueta_final'].replace({"POSITIVO" : 'NR', 
                                                          "NEGATIVO" : 'R',
                                                          "NEUTRAL" :  'NR',
                                                          "NECESIDADES OPERATIVAS": 'R',
                                                          "NO RELEVANTE" : 'NR'})
    ...
    df.to_csv('./fasttext/data/data_' + type_proc + stop_name +'_proc.csv', index=False)
...
filtro = ['NEGATIVO', 'NEUTRAL']

data_extra_neg = data_extra[data_extra.etiqueta_final.isin(filtro)]
#%%
data_final = pd.concat([data, data_extra_neg])    
#%%
data_final['texto_ed'] = clean_text_df(data_final, 'texto', stopwords=False)
#%%
t = proc_etapa0(data_final, type_proc='neg_necop', stop_name='')
#%%
